...
A supported browser query can read a Delta-backed table through signed HTTPS or proxy descriptors and return the same result as native. The current in-repo slice now reaches a narrower but real milestone: it can validate supported browser SQL, bootstrap metadata, deterministically plan/prune the candidate Parquet file set, lower that result into a typed browser execution plan, and execute the curated supported execution-plan corpus locally over loopback-served Parquet files. Broad browser SQL / DataFusion execution remains out of scope.
