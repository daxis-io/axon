...
`apps/axon-web/src/wasm/` is generated by `npm run build:wasm` and is not the source of truth for this document. Artifact size and dependency posture remain tracked in `docs/program/browser-datafusion-size-audit.md`; size measurements are release evidence, not a correctness substitute.
