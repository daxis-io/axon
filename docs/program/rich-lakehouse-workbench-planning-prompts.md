...
```text
You are planning the catalog integration effort (E1) for the Axon web app. We are generalizing Axon into a pluggable provider model. E1 owns the DISCOVERY-ONLY CatalogProvider seam plus the Catalog Explorer surface: DirectUnityCatalog (UC REST), LocalDelta, ObjectStore, and future governed-host discovery implementations that answer "what catalogs/schemas/tables/volumes exist and what is their metadata." Governed-host consumers are downstream integrations, not the organizing model for Axon's open source provider taxonomy. This builds on E0 (state/TanStack Query) and E3A (provider contract messages in proto), and depends on E6 (auth/session) for remote API access.
...
Constraint: never place cloud or catalog secrets in browser code (ADR-0002). Discovery only — keep read resolution + execution out of E1 (E9) and file-level volume access out of E1 (E8). Preserve governed-host compatibility without making a downstream consumer a first-class open source provider taxonomy entry.
...
Context: today the close downstream governed-host integration delegates policy to an external control plane; Axon consumes structured outcomes (sql_fallback_required, blocked) and never makes authorization decisions for that host. Under the pluggable provider model, the standalone profile needs an in-app PDP, while governed-host profiles keep the host authoritative (Cedar advisory).

Read these first: docs/program/browser-uc-brokered-runtime-contract.md (governance/fallback taxonomy); crates/query-contract/src/lib.rs (FallbackReason, BlockedReadPlan, ReadAccessPlanReason); the strategy doc docs/program/rich-lakehouse-workbench-strategy.md (provider seams).

Goal: integrate Cedar (cedar-policy Rust crate compiled to WASM) as a pluggable PDP for client-side authorization in standalone mode (advisory when a governed host remains authoritative). Model entities/schema (principal, catalog, schema, table, volume; actions browse/query/export), wire a decision point into the request path, and map allow/deny onto the existing structured taxonomy.

Key decisions to resolve: (1) Cedar-in-WASM feasibility, bundle-size impact, and whether the PDP runs on the main thread or in the worker; (2) the entity/schema model and how it is populated from E1 catalog metadata and the E6 authenticated principal; (3) where the decision point sits in the request/execution path and how denials map to FallbackReason/BlockedReadPlan; (4) the authoritative-vs-advisory contract per integration profile, and reconciliation with ADR-0008 (amend vs new ADR); (5) how policies are authored/loaded/versioned and whether they sync from a provider.

Constraint: in governed-host deployments, in-app Cedar must never be presented as a substitute for server-side enforcement of governed shapes (row filters, column masks, views).
...
Goal: design the session model for the app shell (UC and governed-host API calls via E1 TanStack Query fetchers using same-origin requests through Envoy with credentials: 'include', 401/redirect-to-login handling, session expiry/refresh surfaced to the UI), AND resolve how the WASM worker's range-read fetches carry the session.
