...
target makes the caller hold one `ResolvedBrowserRead` binding containing an
openable descriptor union, access class, not-after time when one exists, exact
selected and resolved resource identity, provenance, and optional correlation
ID. Target Delta access keeps root-scoped grants root-scoped and wraps existing
`BrowserHttpSnapshotDescriptor` inputs in the per-file compatibility variant.
...
| Compatibility snapshot descriptor | browser-produced or explicit server snapshot resolver response                                                                                                                                                   | app API response or browser runtime                    | `application/json`                                        | memory-only; service responses use `no-store`                                          |
| Data objects                      | public HTTPS, signed HTTPS, or a narrow read proxy                                                                                                                                                               | per-file descriptor URLs or a root-scoped object store | object content type is not semantically important to Axon | durable only for eligible local/public access; governed access defaults to memory-only |
...
correlation to form the `ResolvedBrowserRead` binding. Root-capable sources
produce `RootScopedDelta`; manifests, file actions, and explicit server
snapshot descriptors produce `PerFileSnapshot`. Both enter the same execution
provider without forcing root access through a complete active-file list.
