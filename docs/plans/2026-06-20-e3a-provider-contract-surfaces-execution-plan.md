...
|---|----------|----------------|
| 1 | What E3A delivers | **Messages, not transports.** Proto message contracts for all four seam families + codegen (protobuf-es TS, prost Rust) + a checked-in/drift-gated output, in one Buf module tree. No Connect services for the local seams, no runtime wiring, no TS-mirror retirement (all E3B). |
...
| 6 | Rust backend | **Add a prost backend under Buf** (`protoc-gen-prost` via `buf generate`), verified to build for `wasm32-unknown-unknown`. Confirm prost-vs-alternative is an **E3B** hardening question; E3A only needs the messages to generate and compile. |
...
|---|---|---|
| Buf module + codegen seam | `apps/axon-web/buf.yaml`, `buf.gen.yaml`, `proto/axon/config/v1/settings.proto`, `scripts/config-codegen*.mjs` (E0 deliverables) | The module E3A **extends**. Same `buf.yaml` (add packages), same `buf.gen.yaml` (add prost backend), same `npm run codegen` + `codegen:check` drift gate. |
...
- **WASM artifact budget** (`browser_engine_worker.wasm ≤ 750 KB`, enforced by `tests/perf/report_browser_worker_artifact.sh`): the prost-generated Rust must not bloat the worker. Generated message structs are cheap, but confirm with the existing gate at M3.
...
- **`service QueryEngine`** (defined here, *implemented* in E3B/E9): `Execute (stream events)`, `Preview`, `Cancel`. The Arrow IPC result payload stays an opaque `bytes` field — **not** modeled as proto rows.
...
apps/axon-web/
  ...
  buf.gen.yaml                  # E0; add the prost Rust backend
  proto/axon/
...
crates/
  <contract-proto-crate>/src/generated/       # E3A prost Rust (checked in), or an OUT_DIR build — decide at M1
```
...
- **Rust**: add `protoc-gen-prost` to `buf.gen.yaml`; emit into a contract crate; verify `cargo check --target wasm32-unknown-unknown`.
...
### M0 — Module scaffolding + `axon/common/v1` (no client change)
- Add the new packages to `buf.yaml`; add the prost backend to `buf.gen.yaml`; wire the new dirs into `npm run codegen` + `codegen:check`.
- Define `axon/common/v1` (`ObjectRef`, `Page` convention, `ProviderError`, `Capabilities`) and generate TS + Rust.
- **Gate:** codegen drift + `tsc` + `cargo check` (incl. wasm) green; dependency guardrail re-run for `protoc-gen-prost`.
...
- Run the WASM artifact budget check with the prost types compiled into a throwaway worker build to confirm no budget regression.
...
| **prost bloats the WASM worker** past the 750 KB gate | Med / High | M3 runs the existing artifact budget check with the generated types compiled in; keep messages lean; result payload stays opaque `bytes`. |
...
- **Round-trip parity (the core test):** existing `read-access-plan` + object-grant JSON fixtures decode into generated types and re-encode byte-identically, in **both** TS (protobuf-es JSON) and Rust (prost + serde-json bridge). The same for `browser-sdk` envelope fixtures.
...
- **WASM budget:** M3 builds a throwaway worker with the prost types and runs `report_browser_worker_artifact.sh`; assert no regression.
...
4. **prost vs. alternative Rust backend** — confirm prost under Buf builds for `wasm32-unknown-unknown` at acceptable size; if not, this becomes an E3B toolchain decision and E3A ships TS-only generation as an interim.
