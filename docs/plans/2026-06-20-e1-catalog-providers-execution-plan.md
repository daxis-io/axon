...
> E1 turns today's single-table/runtime catalog posture into one profile of a pluggable `CatalogProvider` seam, and adds a peer "direct UC REST" profile. Governed host products can later plug into the same seam without becoming the organizing model for Axon. It builds directly on the E0 TanStack Query layer (`queryOptions` provider boundary, `queryKeys` factory, infinite queries, `connectionId` cache isolation) and depends on the E6 Envoy session for any authenticated remote read. ADR-0002 is a hard constraint at every milestone: **no cloud or catalog secrets ever reach browser code.**

> **Scope realignment (2026-06-21) — E1 is discovery-only; read resolution + execution moved to [E9](../program/rich-lakehouse-workbench-strategy.md).** The original plan put `resolveTableRead()` on the `CatalogProvider`, extended `QueryTableSource`, and wired the run lifecycle through the catalog seam. That conflated three concerns — _discovery_ (what exists), _read resolution_ (the `ReadAccessPlan`/descriptor/`fallback`/`blocked` family), and _execution_ (run/sample). E1 now owns **discovery + the Catalog Explorer only**; the **DataAccessResolver** seam (read resolution) and the **ExecutionProvider** seam (run/preview) are owned by **E9**. Where sections below still describe `resolveTableRead`/`TableReadResolution`/the `resolved_descriptor` source variant/the read-resolution matrix, that material is **relocated to E9** and is retained here only as a cross-reference; the heading is annotated where this applies. The CatalogProvider's node/metadata types should become **[E3A](../program/rich-lakehouse-workbench-strategy.md)-generated proto messages** once E3A lands, not hand-written TS. Milestone **M3** (read resolution → execution) and the deferred governed-host **M4** move to E9.

> **Governed-consumer deferral (2026-06-20):** governed host integrations are **parked** for E1. The `CatalogProvider` discovery seam, the E9 read-resolution seam (including host-issued `read_access_plan` profiles), and the ADR strategy below intentionally keep close consumers compatible — that design is **not** removed — but E1 does **not** build, ship, or depend on a host-specific integration. The integrated providers for E1 are **DirectUnityCatalog**, **LocalDelta**, and **ObjectStore**. Host-specific work (notably milestone **M4** and its fixtures/tests) is retained below, marked _Deferred_, and can be reactivated without reworking the seam.
...
| 3   | Multi-catalog data model                      | **A `CatalogConnection` registry (E0 client store, persisted) keyed by a stable `connectionId`.** A connection is provider-root-scoped (a UC workspace, a host tenant, a bucket, a local folder), distinct from the _table-scoped_ canonical key in `connect/store.ts`. Hierarchy: `connection → catalog → schema → {table                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | view | volume}`. Active selection `{connectionId, catalog, schema, table}`is **route-owned** (E0 deep link), mirrored into the store. Selection yields a`TableRef`handed to the [E9](../program/rich-lakehouse-workbench-strategy.md) ExecutionProvider (which calls the DataAccessResolver → descriptor → the existing`getSession`/`openDeltaTable` runtime). |
...
| 5   | ADR strategy                                  | **Use ADR-0010 "Pluggable Catalog Providers And Table Read Resolution Seams"** as the boundary ADR. It defines the E1 discovery seam and the E9 read-resolution/execution boundary, reserves governed-host/brokered read-plan profiles, and preserves ADR-0002 as the hard browser-secret constraint. Consumer-specific read-contract ADRs remain load-bearing for downstream release-gate traceability but are not E1 integration targets.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- **Consumer-approved descriptors**: compatibility wrappers can carry embedded `BrowserHttpSnapshotDescriptor` values plus access proof, runtime limits, and runtime-preference metadata, but E1 does not interpret them; E9 owns validation and read resolution.
...
export interface ProviderCapabilities {
  ...
  authority: "browser" | "brokered" | "host"; // who decides read access
}
...
| **Governed host** _(deferred — see deferral note)_ | partial                           | Host catalog/list surfaces (thin; may be navigation-light — list approved/recent tables). Tree nodes can be lazily backed by host list APIs where available.                                                              | Governed-host profile of the E9 DataAccessResolver |
...
The "Read resolution" column is shown only to locate each provider's E9 counterpart; **none of it ships in E1.** Navigation differences (UC is richly browsable; object-store/local are flatter; governed hosts may expose only approved/recent tables) are expressed through `capabilities()` and degrade gracefully in the explorer UI. The DataAccessResolver's generality — `descriptor` for local/object-store/host-approved reads, `read_access_plan` for brokered UC or host-issued plans, `fallback`/`blocked` for governed shapes — is why close consumers can be **deferred without redesign**; see [E9](../program/rich-lakehouse-workbench-strategy.md).
...
Today's `tableSourceKeyFromParts` produces a **table-scoped** canonical key (includes schema + table). E1 needs a **connection-root** id (a UC workspace, a host tenant, a bucket root, a local folder) so the whole `['catalog', connectionId, …]` subtree shares one cache and prefix-invalidates on disconnect.
...
- It declares direct UC catalog listing and future governed-host catalog listing
  as peer discovery profiles, with host-specific implementation deferred.
...
Existing consumer-specific read-contract ADRs carry the governed-host/brokered
profile of this provider model. Do **not** rewrite those contracts in E1 — their
text is referenced by downstream release-gate traceability/proof packets, and
broad edits would churn those gates.
...
### M4 — Governed host consumer profile _(Deferred → [E9](../program/rich-lakehouse-workbench-strategy.md))_

> **Deferred and relocated.** Governed host _discovery_ listing is a deferred profile of the E1 catalog seam; host-issued _read resolution_ (`ReadAccessPlan`/approved descriptors, validation, `limits`/`access_proof`/reason preservation) is a deferred profile of the **E9 DataAccessResolver**. Either way it is **not built, gated, or depended on** in E1. Consumer-specific reactivation details, golden fixtures, and fallback preferences live with E9 and the integration docs.
...
- Land **ADR-0009** (the provider seam, valid independent of any single downstream consumer); keep consumer-specific forward notes deferred with M4. Remove the decorative UC/DS stub text; update `browser-uc-brokered-runtime-contract.md` cross-reference.
...
| **Auth not ready (E6 in flight)**                                                                                         | High / Med  | `SessionHttp` contract + mock UC/broker decouple E1 from E6; DirectUC and governed-host integrations stay flag-gated until E6 lands.                                                                                                  |
...
| **Consumer contract drift vs. generalization** _(deferred with M4)_                                                       | Low / Med   | Host-specific providers are tested against shipped example fixtures. While governed host integration is parked, drift is bounded by leaving consumer fixtures intact and re-validating at reactivation.                               |
...
- **Read-resolution matrix → [E9](../program/rich-lakehouse-workbench-strategy.md).** The DirectUC eligible→`descriptor` / reason→`fallback`/`blocked` matrix, the `openDeltaTable()`-never-for-blocked assertion, and governed-consumer fixture parity are E9's test surface, not E1's.
...
3. **Governed host navigation depth** _(deferred with M4)_ — how browsable should a host-owned catalog listing be in practice (full tree vs. approved/recent tables only)? The answer should preserve a host-agnostic Axon seam.
...
5. **ADR-0010 / consumer-contract ownership** — confirm ADR-0010 as the general provider/read-resolution boundary and keep consumer-specific brokered profiles with the downstream release-gate owners.
