...
Status: landed on `origin/main` through `60f88e7` on 2026-07-16. Slice 1 may
start from a refreshed `origin/main` containing this prerequisite.

- Landed correctness commit `e0e772b` applies `cache: no-store` to the metadata
...
- Landed evidence commit `1e92af3` used the anonymous production-like S3
  fixture in one browser context, performed full page reloads and explicit
  persisted-table reselection between queries, and attached
...
- Landed test-harness commit `60f88e7` gives the Playwright-managed cold build
  and server the same four-minute budget as the live test.
