...
fn datafusion_session_context_constructs_in_wasm() {
    let engine = wasm_datafusion_poc::WasmDataFusionEngine::new();
    ...
    assert_eq!(
        engine.memory_metrics().limit_bytes,
        wasm_datafusion_poc::DEFAULT_BROWSER_DATAFUSION_MEMORY_POOL_BYTES as u64
    );
}
