...
use wasm_parquet_engine::{
    merge_page_index_decision_summaries, merge_parquet_range_read_metrics,
    stream_scan_target_batches_with_adaptive_page_index_policy, AdaptivePageIndexState,
...
struct PageIndexMemoryPressure(bool);

fn parquet_page_index_policy(mode: PageIndexMode) -> ParquetPageIndexPolicy {
    match mode {
        PageIndexMode::Skip => ParquetPageIndexPolicy::Skip,
        PageIndexMode::Predicate => ParquetPageIndexPolicy::Predicate,
        PageIndexMode::Adaptive => ParquetPageIndexPolicy::Adaptive,
    }
}
...
struct AxonParquetScanMetrics {
    ...
    page_index_decision: Option<PageIndexDecisionSummary>,
}
        ...
        Self: 'async_trait,
    {
        Box::pin(async move {
            ...
            let page_index_policy = state
                .config()
                .get_extension::<PageIndexMode>()
                .map(|mode| parquet_page_index_policy(*mode))
                .unwrap_or_default();
            let adaptive_page_index_state = state
                .config()
                .get_extension::<AdaptivePageIndexState>()
                .map(|state| state.as_ref().clone());
            let page_index_memory_pressure = state
                .config()
                .get_extension::<PageIndexMemoryPressure>()
            ...
            let mut scan = AxonParquetScanExec::new(
            ...
            scan.adaptive_page_index_state = adaptive_page_index_state;
            scan.page_index_memory_pressure = page_index_memory_pressure;
            Ok(Arc::new(scan) as Arc<dyn ExecutionPlan>)
        })
    }
...
pub struct AxonParquetScanExec {
    ...
    page_index_memory_pressure: bool,
}
...
impl AxonParquetScanExec {
    fn new(
        ...
        Self {
            ...
            query_context,
            page_index_policy,
            adaptive_page_index_state: None,
            page_index_memory_pressure: false,
        }
    }
    ...
    fn execute_parquet_scan(&self) -> DataFusionResult<SendableRecordBatchStream> {
        ...
        let adaptive_page_index_state = self.adaptive_page_index_state.clone();
        let page_index_memory_pressure = self.page_index_memory_pressure;
        ...
        let parquet_batches = stream::once(async move {
            stream_planned_scan_targets(
                ...
                query_context,
                page_index_policy,
                adaptive_page_index_state,
                page_index_memory_pressure,
            )
            .await
            .map(|scan| {
                scan.batches
...
impl AxonScanPlan {
    fn new(
        ...
        let mut row_group_predicate = filters
            .iter()
            .find_map(|filter| row_group_pruning_predicate(filter, descriptor));
        if conjuncts.len() != 1 {
            if let Some(predicate) = &mut row_group_predicate {
                predicate.page_index_supported = false;
            }
        }
...
fn record_scan_parquet_metrics(
    ...
    scan_metrics.page_index_decision = snapshot.page_index_decision;
}
...
fn add_scan_metrics(
    ...
    ScanTargetMetricsSnapshot {
        ...
        page_index_decision: merge_page_index_decision_summaries(
            left.page_index_decision,
            right.page_index_decision,
        ),
    }
}
...
async fn stream_planned_scan_targets(
    ...
    let batches = stream::try_unfold(state, |mut state| async move {
        loop {
            ...
            let scan = stream_scan_target_batches_with_adaptive_page_index_policy(
                &state.reader,
                &target,
                ...
                state.adaptive_page_index_state.as_ref(),
                state.page_index_memory_pressure,
            )
            .await?;
            state.cancellation.check_cancelled_at("scan stream")?;
            let metrics_snapshot =
...
fn query_budget_exceeded_error_at(
    ...
    match budget_name {
        ...
        _ => QueryError::new(QueryErrorCode::FallbackRequired, message, runtime_target())
...
impl WasmDataFusionEngine {
    ...
    pub fn with_budget_and_memory_limit(
        ...
        Self::with_budget_cancellation_and_caches_and_memory_limit(
            query_budget,
            BrowserQueryCancellation::default(),
            ParquetMetadataCache::default(),
            ParquetRangeCache::default(),
    ...
    pub fn with_budget_cancellation_and_caches_and_memory_limit(
        query_budget: BrowserQueryBudget,
        cancellation: BrowserQueryCancellation,
        metadata_cache: ParquetMetadataCache,
        range_cache: ParquetRangeCache,
    ...
    fn with_budget_cancellation_caches_and_memory_limit(
        ...
        Self {
            ...
            adaptive_page_index_state: AdaptivePageIndexState::default(),
            spill_execution_id: None,
        }
    }
    ...
    /// Selects the worker-local spill execution used for the next query.
    /// Execution id zero installs an unavailable backend so small queries can
    /// proceed while a real spill reports `spill_storage/unavailable`.
    pub fn set_external_memory_execution(&mut self, execution_id: Option<u32>) -> Option<u32> {
        std::mem::replace(&mut self.spill_execution_id, execution_id)
    }
    ...
    pub fn set_page_index_mode(
        ...
        self.adaptive_page_index_state
            .set_calibration_error_margin_us(calibration_error_margin_us)?;
        Ok(std::mem::replace(
            &mut self.page_index_mode,
            page_index_mode,
        ))
    }
    ...
    pub fn set_page_index_policy(
        ...
        let previous = parquet_page_index_policy(self.page_index_mode);
        self.page_index_mode = match page_index_policy {
    ...
    fn begin_query_session(
        ...
        state
            .config_mut()
            .set_extension(Arc::new(self.page_index_mode));
        state
            .config_mut()
            .set_extension(Arc::new(self.adaptive_page_index_state.clone()));
        let memory = self.memory_pool.metrics();
        let memory_pressure =
            memory.reserved_bytes.saturating_mul(10) >= memory.limit_bytes.saturating_mul(9);
        state
            .config_mut()
        ...
        if let Some(execution_id) = self.spill_execution_id {
            ...
            let spill_storage: Arc<dyn datafusion::execution::spill_storage::SpillStorage> =
                browser_spill::BrowserOpfsSpillStorage::new(execution_id, memory_pool);
            let runtime = RuntimeEnvBuilder::from_runtime_env(state.runtime_env())
                .with_spill_storage(spill_storage)
                .build()
                .expect("execution-scoped browser spill runtime should construct");
...
fn add_scan_trace_to_datafusion_metrics(
    ...
    metrics.page_index_decision =
        merge_page_index_decision_summaries(metrics.page_index_decision, trace.page_index_decision);
...
fn map_datafusion_error(error: datafusion::error::DataFusionError) -> QueryError {
    ...
    if error_text.contains("spill_storage/io_failure") {
        let operation = [
            "create_scope",
            "create_writer",
            "open_reader",
        ...
        .into_iter()
        .find(|operation| error_text.contains(operation));
        let message = operation.map_or_else(
            || "browser aggregate spill storage I/O failed".to_owned(),
            |operation| format!("browser aggregate spill storage I/O failed during {operation}"),
        );
        return QueryError::new(QueryErrorCode::ResourceExhausted, message, runtime_target())
            .with_resource_details(QueryResourceDetails {
                resource: QueryResource::SpillStorage,
                reason: QueryResourceReason::IoFailure,
            });
    }
    if error_text.contains("spill_storage/quota_exceeded") {
        return resource_exhausted_from_datafusion_message("spill_storage/quota_exceeded");
    }
    if error_text.contains("spill_storage/unavailable") {
        return resource_exhausted_from_datafusion_message("spill_storage/unavailable");
    }
    if error_text.contains("operator_memory/quota_exceeded") {
        return resource_exhausted_from_datafusion_message("operator_memory/quota_exceeded");
    }

    // Classify on the root error. DataFusion wraps failures raised deep in an operator with
    // context, and a wrapped ResourcesExhausted still means the browser hit a
    // bounded resource limit rather than an opaque execution failure.
    match error.find_root() {
        ...
        DataFusionError::ResourcesExhausted(message) => {
            resource_exhausted_from_datafusion_message(message)
        }
        _ => QueryError::new(
            QueryErrorCode::ExecutionFailed,
...
fn resource_exhausted_from_datafusion_message(message: &str) -> QueryError {
    let (resource, reason, public_message) = if message.contains("spill_storage/unavailable") {
        (
            QueryResource::SpillStorage,
            QueryResourceReason::Unavailable,
            "browser aggregate spill storage is unavailable".to_owned(),
        )
    } else if message.contains("spill_storage/quota_exceeded") {
        (
            QueryResource::SpillStorage,
            QueryResourceReason::QuotaExceeded,
            "browser aggregate spill storage quota was exceeded".to_owned(),
        )
    } else {
        (
            QueryResource::OperatorMemory,
            QueryResourceReason::QuotaExceeded,
    ...
    .with_resource_details(QueryResourceDetails { resource, reason })
}
...
mod tests {
    ...
    #[test]
    fn compound_filters_keep_row_group_pruning_but_reject_page_index_selection() {
        ...
        let descriptor = DeltaTableDescriptor {
            ...
            active_files: vec![DeltaActiveFile {
                ...
                object_etag: Some("\"stable\"".to_string()),
                stats_json: None,
                deletion_vector: None,
            }],
        };
        let simple = AxonScanPlan::new(
            &descriptor,
            &batch.schema(),
            &[col("id").gt(lit(1_i32))],
            None,
        ...
        assert!(simple
            .row_group_predicate
            .as_ref()
            .is_some_and(|predicate| predicate.page_index_supported));

        let compound = AxonScanPlan::new(
            &descriptor,
            &batch.schema(),
            &[col("id").gt(lit(1_i32)).and(col("value").lt(lit(20_i32)))],
            None,
            &[],
        );
        assert!(compound.row_group_predicate.is_some());
        assert!(!compound
            .row_group_predicate
            .as_ref()
            .is_some_and(|predicate| predicate.page_index_supported));
