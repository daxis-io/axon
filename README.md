...
- `BrowserRuntimeSession::execute_plan(&self, snapshot, plan)` now executes the current supported execution-plan subset against materialized browser HTTP objects by reading the planned candidate Parquet files, synthesizing partition columns from per-file `partition_values`, applying typed row filters, projecting passthrough outputs, executing narrow grouped and ungrouped aggregates, and preserving output-aligned `ORDER BY` / `LIMIT`. Deferred aggregate functions such as `ARRAY_AGG`, `BOOL_AND`, and `BOOL_OR` return structured native fallback instead of overclaiming browser support.
