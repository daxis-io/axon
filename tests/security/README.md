...
- The Daxis-facing app worker is browser DataFusion-backed. The legacy narrow runtime plus streaming scan plus in-memory session shell remains compatibility-only, and OPFS / IndexedDB browser-cache backends are still deferred.
- `tests/security/verify_browser_dependency_guardrails.sh` denies signing, cloud-credential, and service-account dependency classes from the browser worker dependency tree, checks browser package manifests for AWS/GCP/Azure SDKs, and inspects the built worker artifact for secret-like markers. Set `AXON_BROWSER_DEPENDENCY_PACKAGE` to target a specific browser package; the Daxis release evidence runner uses `axon-web-wasm` with the built `axon_web_wasm.wasm` artifact.
...
This directory still does not contain service-level EPIC-03 security checks because `services/query-api` is not in this repository. Signed URL issuance, proxy-mode request issuance, audit logging, and production-shape CORS/origin validation remain external blockers outside repo-owned browser-engine success claims.
