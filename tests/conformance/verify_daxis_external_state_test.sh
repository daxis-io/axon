...
artifact_path="$tmpdir/external-state.json"
write_output="$(run_helper --write-json "$artifact_path")"
if [[ ! -f "$artifact_path" ]]; then
	echo "expected Daxis external-state helper to write JSON artifact" >&2
	exit 1
fi
artifact_sha256="$(shasum -a 256 "$artifact_path" | awk '{print $1}')"
if [[ "$write_output" != *"daxis_external_state_json_path=$artifact_path"* ]]; then
	echo "expected Daxis external-state helper to print JSON artifact path" >&2
	exit 1
fi
if [[ "$write_output" != *"daxis_external_state_json_sha256=$artifact_sha256"* ]]; then
	echo "expected Daxis external-state helper to print JSON artifact SHA-256" >&2
	exit 1
fi
