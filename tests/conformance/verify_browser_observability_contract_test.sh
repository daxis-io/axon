...
contract="$repo_root/docs/program/browser-observability-contract.md"

mkdir -p "$repo_root/docs/program"
...
- `row_groups_touched`: Parquet row groups decoded by the browser scan layer when tracked
- `row_groups_skipped`: Parquet row groups skipped by browser row-group pruning when tracked
- `footer_reads`: browser snapshot-bootstrap footer reads when tracked
- `rows_emitted`: rows emitted by the scan layer
...
verify_fixture() {
  AXON_BROWSER_OBSERVABILITY_CONTRACT_REPO_ROOT="$repo_root" \
    bash tests/conformance/verify_browser_observability_contract.sh >/dev/null 2>&1
}
...
PY
  ...
  if verify_fixture; then
    echo "expected missing browser observability $description to be rejected" >&2
    exit 1
  fi
}

expect_missing_text_rejected "- fallback reason" "dashboard fallback signal"
expect_missing_text_rejected '`arrow_ipc_bytes`' "Arrow IPC byte metric"
expect_missing_text_rejected "## Daxis Result Metrics Mapping" "Daxis result metrics section"
expect_missing_text_rejected '`rows_returned`: maps from `QueryMetricsSummary.rows_emitted`' "Daxis rows-returned mapping"
expect_missing_text_rejected '`scan_bytes`: maps from `QueryMetricsSummary.bytes_fetched`' "Daxis scan-bytes mapping"
expect_missing_text_rejected 'Fallback, rejection, failure, and cancellation envelopes keep `metrics` as an object' "Daxis non-executed metrics boundary"
expect_missing_text_rejected '`query_id`' "worker event query id"
expect_missing_text_rejected '`crates/browser-sdk` preserves those fields through the worker envelope.' "SDK evidence source"
expect_missing_text_rejected "No control-plane or service-level request correlation exists in this repository." "non-claim"
expect_missing_text_rejected "Daxis-owned tenant, workspace, catalog, policy, access-mode, audit, rollout, and oncall context" "Daxis handoff context"

rm -f "$contract"

if verify_fixture; then
  echo "expected missing browser observability contract to be rejected" >&2
  exit 1
fi

echo "Browser observability contract verifier regression coverage passed"
