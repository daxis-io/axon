...
- CI compiles `wasm-http-object-store`, `wasm-parquet-engine`, `wasm-delta-snapshot`, `wasm-query-runtime`, and `browser-sdk` for `wasm32-unknown-unknown`, runs host tests for the new browser-side crates, and keeps the existing `wasm-query-runtime` `wasm32-unknown-unknown` smoke test.
